...
> 当前仓库负责上位机、视觉识别、棋局建模、AI 决策与棋谱工具。机械臂底层控制已拆分到独立仓库：[gomoku-arm-controller](https://github.com/KITTIYOJIANG/gomoku-arm-controller)，本地路径为 `D:\Projects\gomoku_arm_controller`。
...
独立下位机仓库 [gomoku-arm-controller](https://github.com/KITTIYOJIANG/gomoku-arm-controller) 负责：
...
- 将机械臂底层控制从上位机仓库中解耦，设计为独立下位机控制仓库 `gomoku-arm-controller`，通过抽象棋盘坐标 `(row, col)` 完成上位机与 STM32 机械臂控制层的边界划分。
