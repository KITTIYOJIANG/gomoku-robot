...
> 当前仓库负责上位机、视觉识别、棋局建模、AI 决策与棋谱工具。机械臂底层控制目前在本地项目 `D:\Projects\gomoku_arm_controller` 中开发，暂未建立 GitHub 远程仓库。
...
本地下位机控制项目 `D:\Projects\gomoku_arm_controller` 负责：
...
- 将机械臂底层控制从上位机仓库中解耦，设计为本地独立下位机控制项目 `gomoku_arm_controller`，通过抽象棋盘坐标 `(row, col)` 完成上位机与 STM32 机械臂控制层的边界划分。
...
- Add integration examples with local `gomoku_arm_controller`
